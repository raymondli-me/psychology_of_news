#!/usr/bin/env python3
"""Process YouTube data quickly (skip slow UMAP, use PCA instead)."""
...
def main():
    ...
    if not raw_path.exists():
        ...
        print("Run: python -c \"from collectors.youtube import collect_youtube; collect_youtube('draymond green trade', output_path='youtube/data/raw_youtube.json')\"")
        return
    ...
    # Cluster with KMeans
    print("Clustering...")
    n_clusters = min(8, len(raw_data) // 10 + 1)  # Adaptive cluster count
    ...
    # Generate initial topic names based on cluster content
    ...
    for model in ["gpt", "claude", "gemini"]:
        ...
        for cluster_id in range(n_clusters):
            ...
            if cluster_points:
                ...
                elif "suspension" in all_text or "punch" in all_text or "ejected" in all_text:
                    name = "Suspensions & Drama"
                elif "trade" in all_text and ("rumor" in all_text or "news" in all_text):
                    name = "Trade News"
                elif "dynasty" in all_text or "championship" in all_text or "ring" in all_text:
                    name = "Dynasty Talk"
                elif "contract" in all_text or "salary" in all_text or "money" in all_text:
                    name = "Contract Discussion"
                elif "steph" in all_text or "curry" in all_text:
                    name = "Curry Connection"
                elif "mavs" in all_text or "dallas" in all_text or "mavericks" in all_text:
                    name = "Mavs Interest"
                elif "highlights" in all_text or "best" in all_text or "top" in all_text:
                    name = "Highlights"
                elif "podcast" in all_text or "show" in all_text or "discuss" in all_text:
                    name = "Podcast/Analysis"
                else:
                    # Use most common channel as topic
                    channels = [p["channel_name"] for p in cluster_points]
                    most_common = max(set(channels), key=channels.count)
                    name = f"{most_common[:20]}"
    ...
    print("  python youtube/server.py")
